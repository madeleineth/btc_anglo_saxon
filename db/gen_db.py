...
def add_inflected_terms_to_db(
    inflections_path: str, db: Connection, limit: Optional[int] = None
) -> None:
        ...
        try:
            ...
            for row in rd:
                ...
                if len(term) > 0 and len(join_term) > 0:
                    c.execute("INSERT INTO idx_tmp VALUES (?, ?)", (term, join_term))
                if n % 100000 == 0:
            ...
            c.execute(
                "CREATE TEMP TABLE idx_new AS SELECT DISTINCT "
            ...
            logging.info("{:,} inflected terms added, {:,} unmatched.".format(n_new, n_unmatched))
        ...
        with open(extra_forms_path, "rt", encoding="UTF-8") as f:
            for line in f:
                ...
                for form in forms.split(","):
                    c.execute("INSERT INTO idx_tmp VALUES (?, ?)", (ascify(form), ascify(headword)))
                    n += 1
        c.execute(
            "INSERT INTO idx SELECT DISTINCT idx_tmp.term, idx.nid "
...
def read_abbrevs_and_add_to_db(abbrevs_path: str, db: Connection) -> Dict[str, int]:
...
def write_rev_file(output_base_path: str, nbytes: int = 16) -> None:
...
def linkify(defn: str, term_nid: Dict[str, int], rex: Any, current_nid: int, skip: int) -> str:
    ...
    def preceded_by_v(m: re.Match[str]) -> bool:
        return m.start(0) >= 3 and m.string[(m.start(0) - 3) : m.start(0)] == "v. "

    def replace(m: re.Match[str]) -> str:
        assert len(m[0]) > 0, "m = %r, should have nonzero length" % (m,)
        if (
            m[0].startswith("<")
            or (m.start(0) < skip and not preceded_by_v(m) and " " not in m[0])
...
def linkify_and_normalize_defns(
    db: Connection, term_nid: Dict[str, int], abbrevs: Iterable[str]
) -> None:
...
def main() -> None:
    args = parse_args()
    logging.basicConfig(
        format="%(asctime)s: %(message)s", level=logging.INFO, datefmt="%Y-%m-%dT%H:%M:%S"
    )

    with open(args.mod_eng_dictionary, "rt") as mod_eng_dict:
        mod_e_words = mod_eng_dict.read().strip().split()
        exclude_words = set(mod_e_words).union(set(EXTRA_MOD_ENG_WORDS)).union(VERY_COMMON_WORDS)
